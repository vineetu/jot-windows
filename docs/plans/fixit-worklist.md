...
## B. Other shown-but-broken (found in audit; user didn't flag but should decide)

- **B1. Vocabulary / custom terms is fake — L or S.** `SettingsPage.xaml:197-247` binds to an in-memory
  list seeded `["Jot","Nemotron","WASAPI"]` (`SettingsViewModel.cs:383`); never persisted, never read by
  any transcriber. **Either** make it real (persist + bias the decoder — this is L and matches a real Mac
  feature) **or** hide the section until then (S). Don't ship a fake toggle.
- **B2. "Detect speakers" is a stub — L.** `RecordingDetailViewModel.cs:184-199` fakes speakers by
  alternating on sentence boundaries; exported VTT inherits the fake turns. Either implement real
  diarization (L) or relabel/hide.
- **B3. "Check for updates" is hardcoded — M.** `AboutPage.xaml.cs:12-15` always says "latest build," no
  network check. Tie into the planned Velopack auto-update.
